"""
LangGraph Checkpoint Management and Message Streaming

This module handles checkpoint persistence and message streaming for LangGraph workflows.
It supports multiple storage backends (PostgreSQL, MongoDB, in-memory) and provides
streaming functionality for real-time conversation updates.

Functions:
    chat_stream_message: Stream chat messages with persistence
    save_postgres_message: Save messages to PostgreSQL database
    save_mongodb_message: Save messages to MongoDB database  
    save_memory_message: Save messages to in-memory storage
    get_postgres_chat_history: Retrieve chat history from PostgreSQL
    get_mongodb_chat_history: Retrieve chat history from MongoDB
    get_memory_chat_history: Retrieve chat history from memory

Supports conversation persistence across different storage backends with automatic
failover to in-memory storage when external databases are unavailable. Includes
message streaming capabilities for real-time UI updates.
"""
...
from src.deep_research.config.configuration import get_bool_env, get_str_env
...
class ChatStreamManager:
    ...
    def __init__(
        ...
        if self.checkpoint_saver:
            ...
            self.logger.debug("Checkpoint saver is disabled")
...
def chat_stream_message(thread_id: str, message: str, finish_reason: str) -> bool:
    ...
    if checkpoint_saver:
        ...
        logging.debug("Checkpoint saver is disabled, message not processed")
        return False
